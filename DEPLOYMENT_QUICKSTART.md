...
### Linux/macOS:
```bash
# Прямой запуск (порт по умолчанию 8000)
./deploy.sh direct

# На другом порту (если 8000 занят)
SERVER_PORT=8001 ./deploy.sh direct

# Docker
./deploy.sh docker

# Docker на другом порту
SERVER_PORT=8001 ./deploy.sh docker

# Systemd (требует sudo)
sudo ./deploy.sh systemd

# Systemd на другом порту
SERVER_PORT=8001 sudo ./deploy.sh systemd
```
...
# На другом порту
$env:SERVER_PORT=8001; .\deploy.ps1 direct

# Docker
.\deploy.ps1 docker

# Docker на другом порту
$env:SERVER_PORT=8001; .\deploy.ps1 docker
```

### Если порт занят:
Скрипты автоматически проверяют, занят ли порт, и предупреждают об этом. Используйте переменную окружения `SERVER_PORT` для выбора другого порта.
