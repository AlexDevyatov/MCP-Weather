"""MCP-сервер для получения данных о погоде через Яндекс Погода API."""
import asyncio
import logging
import sys
        ...
        types.Tool(
            ...
            input_schema={
                ...
                "properties": {
                    "days": {
                        "type": "integer",
                        "description": "Количество дней прогноза (по умолчанию 3, максимум 7)",
                        "default": 3,
                        "minimum": 1,
                        "maximum": 7
                    },
...
async def call_tool(name: str, arguments: dict[str, Any]) -> list[types.ContentBlock]:
    ...
    try:
        if name == "get_current_weather":
            ...
            days = min(max(int(arguments.get("days", 3)), 1), 7)
            ...
            if result:
                ...
                return [types.TextContent(
                    type="text",
                    text=f"Город '{city_name}' не найден в базе"
                )]
